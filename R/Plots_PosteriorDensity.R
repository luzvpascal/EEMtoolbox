Plots_PosteriorDensity <- function(prior_sample, posterior_sample, prior_args,param_names,max_per_figure){
  ...
  for (i in seq(n_params)){
    dens_prior <- density(prior_sample[,i],bw = "nrd")
    ...
    dens_posterior <- density(posterior_sample[,i],bw = "nrd")
  ...
  g <- ggplot(data_densities, aes(x=xx,
                             y=yy,
